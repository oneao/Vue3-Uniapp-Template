...
function encode(src: string, rfc4648 = false): string {
...
interface OptionsType {
  msg: string
  duration?: number // 修正 `druation` 并改为可选，避免必须传入
  image?: string
}
...
export function UniCustomToast() {
  // 统一处理 options
  function normalizeOptions(options: string | OptionsType): OptionsType {
    return typeof options === 'string'
      ? { msg: options, duration: 2000 }
      : { duration: 2000, ...options }
  }

  // 通用的 toast 逻辑
  function showToast(options: OptionsType, type?: keyof typeof toastIcon) {
    const { msg, duration, image } = options
    uni.showToast({
      title: msg,
      duration,
      icon: 'none',
      image: image?.trim()
        ? image
        : type
          ? `data:image/svg+xml;base64,${encode(toastIcon[type]())}`
          : 'none',
    })
  }

  // 通用的 show 方法
  function show(options: string | OptionsType) {
    showToast(normalizeOptions(options))
  }

  // 成功提示
  function success(options: string | OptionsType) {
    showToast(normalizeOptions(options), 'success')
  }

  // 警告提示
  function warning(options: string | OptionsType) {
    showToast(normalizeOptions(options), 'warning')
  }

  // 信息提示
  function info(options: string | OptionsType) {
    showToast(normalizeOptions(options), 'info')
  }

  // 错误提示
  function error(options: string | OptionsType) {
    showToast(normalizeOptions(options), 'error')
  }

  return { show, success, warning, info, error }
}
